<?php
require_once "../Conn/Conn.php";

$themeId = isset($_GET['id']) ? $_GET['id'] : '';
if(strlen($themeId) > 0){
    $connect = connect();
    $info = $info2 = $info3 = array();
    //专题
    $sql = "SELECT TOP 1 [title],[description],[coverImg],[keywords] FROM [theme] WHERE [themeId]='" . $themeId ."' AND [isDel] = 0";
    $result = sqlsrv_query($connect, $sql);
    $info = sqlsrv_fetch_array($result);
    //专题文章
    $sql2 = "SELECT TOP 5 [themeAid], [foreword], [themeTitle], [coverImg], [message] FROM [themeArticle] WHERE [themeId]='" . $themeId . "' AND [isUp] = 1 AND [isDel] = 0 ORDER BY [createTime] DESC";
    $result2 = sqlsrv_query($connect, $sql2);
    while($row2 = sqlsrv_fetch_array($result2)){
        $row2['coverImg'] = str_replace('http://img1.financeun.com','http://img.financeun.com',$row2['coverImg']);//兼容旧版本图片处理
        if(strpos($row2['coverImg'], 'http://') === false){
            $row2['coverImg'] = 'http://img.financeun.com' . $row2['coverImg'];
        } else{
            $row2['coverImg'] = $row2['coverImg'];
        }
        $row2['foreword'] = mb_substr($row2['foreword'],0,30);//截取导读前30个汉字
        $info2[] = $row2;
    }
    //相关文章
    $kwArr = explode(",", $info['keywords']);
    $str = "";
    for($i=0,$len=count($kwArr);$i<$len;$i++){
        $str .= " Title LIKE '%" . $kwArr[$i] . "%' OR";//拼接关键词模糊查询
    }
    $str = rtrim($str, 'OR');
    $sql3 = "SELECT TOP 10 [Aid], [Title], [CoverImg], [Tag], [source], [ArticleType], [visit], [visitshow], [CreateTime] ";
    $sql3 .= "FROM [ArticleInfo] WHERE ";
    if($maxCreateTime == '' .. $minCreateTime != ''){//最旧文章时间不为空
        $sql3 .= "DATEDIFF(SECOND,CONVERT(datetime,'" . date("Y-m-d H:i:s", $minCreateTime) . "',101), [CreateTime]) < 0 AND ";
    } elseif($maxCreateTime != '' .. $minCreateTime == ''){//最新文章时间不为空
        $sql3 .= "DATEDIFF(SECOND,CONVERT(datetime,'" . date("Y-m-d H:i:s", $maxCreateTime) . "',101), [CreateTime]) > 0 AND ";
    }
    $sql3 .= "[isUP] = 1 AND [isDel] = 0 AND " . $str . " ORDER BY [CreateTime] DESC";
    $result3 = sqlsrv_query($connect, $sql3);
    while($row3 = sqlsrv_fetch_array($result3)){
        $row3['CoverImg'] = str_replace('http://img1.financeun.com','http://img.financeun.com',$row3['CoverImg']);//兼容旧版本图片处理
        if(strpos($row3['CoverImg'], 'http://') === false){
            $data[$i]['CoverImg'] = 'http://img.financeun.com' . $row3['CoverImg'];
        } else{
            $data[$i]['CoverImg'] = $row3['CoverImg'];
        }
        $info3[] = $row3;
    }
    sqlsrv_close($connect);
}
?>

<!doctype html>
<html>
<head>
	<meta charset="UTF-8">
	<meta http-equiv="X-UA-Compatible" content="IE=edge">
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<meta name="apple-mobile-web-app-capable" content="yes">
	<meta name="description" content="<?php echo $info['description'];?>">
	<meta name="shareicon" content="<?php echo $info['coverImg'];?>">
	<title><?php echo $info['title'];?></title>
	<link rel="stylesheet" type="text/css" href="http://192.168.0.69/FinanceDisk/Finance-webApp/News/Css/themeShare_common.css">
    <link rel="stylesheet" type="text/css" href="http://192.168.0.69/FinanceDisk/Finance-webApp/News/Css/swiper.min.css">
    <script type="text/javascript" src="http://192.168.0.69/FinanceDisk/Finance-webApp/News/Js/jquery-3.1.1.min.js"></script>
    <script type="text/javascript" src="http://192.168.0.69/FinanceDisk/Finance-webApp/News/Js/swiper.jquery.min.js"></script>
    <script type="text/javascript" src="http://192.168.0.69/FinanceDisk/Finance-webApp/News/Js/swipe.js"></script>
</head>
<body class="page-swipe">
<section>
	<?php
	$coverImg = $info['coverImg'];
	if(stripos($coverImg , "http://") === false){
		$coverImg = "http://img.financeun.com" . $coverImg;
	}
	?>
    <!-- 顶图 -->
	<div class="section">
	    <img src='http://192.168.0.69/FinanceDisk/Finance-webApp/images/1.jpg' height="260"/>
	</div>
    <!-- 描述 -->
    <div class="descrip"><?php echo $info['description'];?></div>
    <div class="nav_bar"><span style="color:red;">|</span>特别关注</div>
    <!-- 专题文章轮播和滑动图 -->
    <div id="slider" class="swipe" style="visibility:visible;">
            <div class="swipe-wrap">
                <?php foreach($info2 as $k=>$v){
                $str  = '';
                $str .= '<figure>';
                $str .=     '<div class="face faceInner">';
                $str .=         '<a href="http://app.financeun.com/News/ThemeArt/' . $v['themeAid'] . '">';
                $str .=         '<div class="box">';
                $str .=             '<div>';
                $str .=                 '<img src="' . $info3[0]['CoverImg'] . '" style="width: 35%;border-radius:3px;box-sizing:border-box;float:left">';
                $str .=             '</div>';
                $str .=             '<div style="float:left;width:65%;padding-left:5px;">';
                $str .=                 '<div style="font-size:14px;font-weight:bold;color:#000;">' . $v['themeTitle'] . '</div>';
                $str .=                 '<div style="font-size:12px;color:#a2a2a2;">' . $v['foreword'] . '...</div>';
                $str .=             '</div>';
                $str .=             '<div class="clear"></div>';
                $str .=         '</div>';
                $str .=         '</a>';
                $str .=     '</div>';
                $str .= '</figure>';
                echo $str;
                }
                ?>
            </div>
    </div>
    <nav>
        <ul id="position">
            <li class="on"></li>
            <li class=""></li>
            <li class=""></li>
            <li class=""></li>
            <li class=""></li>
        </ul>
    </nav>
    <script>
        var slider =
            Swipe(document.getElementById('slider'), {
                auto: 3000,
                continuous: true,
                callback: function(pos) {
                    var i = bullets.length;
                    while(i--){
                        bullets[i].className = ' ';
                    }
                    bullets[pos].className = 'on';
                }
            });
        var bullets = document.getElementById('position').getElementsByTagName('li');
    </script>
    <!-- 广告 -->
    <div class="section">
        <a href="http://a.app.qq.com/o/simple.jsp?pkgname=com.NationalPhotograpy.weishoot">
            <img src="http://weishoot.com/Content/story/images/ad1.jpg" width="100%" style="padding:5px 0;">
        </a>
    </div>
    <div class="nav_bar"><span style="color:red;">|</span>最新文章</div>
    <!-- 相关文章列表 -->
    <ul>
    <?php
    foreach($info3 as $k=>$v){
        $str  = '';
        $str .= "<a href='http://app.financeun.com/News/Article/" . $v['Aid'] . "'>";
        $str .= "<li class='list'>";
        $str .=     "<div class='box'>";
        $str .=         "<div>";
        $str .=             "<img src='" . $v['CoverImg'] . "'>";
        $str .=         "</div>";
        $str .=         "<div style='float:left;width:65%;padding-left:5px;'>";
        $str .=             "<div>" . $v['Title'] . "</div>";
        $str .=         "</div>";
        $str .=         "<div class='theme'>" . $info['title'] . "专题</div>";
        $str .=         "<div class='clear'></div>";
        $str .=     "</div>";
        $str .= "</li>";
        $str .= "</a>";
        echo $str;
    }
    ?>
    </ul>
</section>
<div style="height:80px;">
</div>
<!-- 下载应用条 -->
<div class="downApp">
    <img src="data:image/png;base64," width="168">
    <a href="http://a.app.qq.com/o/simple.jsp?pkgname=com.financeun.finance">.#19979;.#36733;</a>
</div>
</body>
</html>
